# OrbitIQ — Production Buildout Plan

> **Created: 2026-07-24**
> **Goal:** Replace all mock/stub implementations with real, production-quality code across 5 phases.

---

## Guiding Principles

1. **Never ship mock data to production.** Every feature must trace to a real engine, real database, or real API call.
2. **Build for observability from day one.** Every new subsystem must expose metrics, structured logs, and health checks.
3. **Golden-rule test:** Ask "Would this pass a 5-minute demo to a Fortune 500 CTO?" If no, it's not ready.
4. **Guardrails are not optional.** Real RLS, real CLS, real audit logs, real prompt-injection defense are non-negotiable before GA.
5. **Build one vertical slice fully before starting the next.** Avoid horizontal "almost-works-everywhere."
...
┌─────────────────────────────────────────────────────────────────┐
│                        Frontend (Next.js 15)                    │
│  Dashboard • Explore • OQL IDE • AI Assistant • Admin • Embed   │
└──────────────┬──────────────────────────────────┬───────────────┘
               │ GraphQL (Apollo Client)          │ REST/SSE
               ▼                                  ▼
┌──────────────────────────────────┐  ┌──────────────────────────┐
│     API Gateway (NestJS)         │  │  Analytics Engine (FastAPI)│
│  Semantic models • RBAC • OQL    │  │  Forecast • Stats • ML    │
│  Compliance • Audit • Sharing    │  │  Celery workers • MLflow  │
└──────────┬───────────────────────┘  └──────────┬───────────────┘
           │                                      │
           ▼                                      ▼
┌─────────────────────────────────────────────────────────────────┐
│                    Query Federation Layer                        │
│  DuckDB (in-process) • Trino (big-data) • ClickHouse (stream)  │
└──────────┬───────────────────────┬──────────────┬──────────────┘
           │                       │              │
     ┌─────▼─────┐           ┌────▼────┐    ┌────▼────┐
     │ PostgreSQL │           │  MySQL  │    │ S3/GCS  │
...
**Goal:** User can upload an Excel file or connect a real Postgres database and see real sampled data + inferred schema in Explore.
...
**Goal:** Dashboard tile shows real numbers from real sources, RLS enforced, complex measures work.
...
**Goal:** Forecast on real data returns backtested, verifiably correct numbers.
...
**Goal:** NL question against real dataset produces correct chart matching hand-built query.
...
**Goal:** Real load tests, real security audit, GA checklist against real subsystems.
...
Phase A (Real Data In)
    ...
    └── A.5 DuckDB Setup ─────────┘
                                  │
                                  ▼
Phase B (Real Query Path)         │
    │                             │
    ├── B.1 OQL DAX Engine ──────┐
    ├── B.4 Query Federation ────┤
    └── B.5 Governance ──────────┘
                                  │
                                  ▼
...
|------|--------|------------|-------|
| OQL compiler lacks CALCULATE/filter-context | Gates all complex measure claims | Audit immediately, prioritize rewrite | B |
| File ingestion scope creep | Timeline slip | Focus on CSV + Excel first, add formats later | A |
| Real LLM costs for agent testing | Budget | Use Ollama for dev, budget Anthropic for staging | D |
| Trino cluster complexity | Operational overhead | Start with DuckDB-only for Phase B, add Trino in Phase E | E |
| Embedding costs for semantic matching | Ongoing cost | Start with text-embedding-3-small, upgrade if needed | D |

---

## Success Metrics

| Metric | Current (POC) | Target (GA) |
|--------|---------------|-------------|
| Connector fidelity | Mock (seed data) | Real driver, real data |
| OQL measure types | 5 basic (SUM/AVG/COUNT/MIN/MAX) | 25+ (DAX-equivalent) |
| Forecast backtesting | None | Real RMSE/MAPE on held-out data |
| RLS enforcement | UI-level filtering only | SQL predicate injection |
| CLS enforcement | Frontend masking only | Query engine projection masking |
| AI agent accuracy | Pattern-matching only | LLM-powered, tool-calling loop |
| Load test results | Simulated numbers | Real 10K concurrent users |
| Security posture | UI-only checks | Real pen test, WORM audit logs |
